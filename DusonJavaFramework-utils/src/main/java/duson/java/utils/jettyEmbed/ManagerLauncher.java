...
package duson.java.utils.jettyEmbed;
...
public class ManagerLauncher {
    ...
    private static final Logger logger = LoggerFactory.getLogger(ManagerLauncher.class);
    private static final String CLASSPATH_URL_PREFIX = "classpath:";

    public static void main(String[] args) throws Throwable {
        try {
            String conf = System.getProperty("*.conf", "classpath:*.properties");
            Properties properties = new Properties();
            if (conf.startsWith(CLASSPATH_URL_PREFIX)) {
                conf = StringUtils.substringAfter(conf, CLASSPATH_URL_PREFIX);
                properties.load(ManagerLauncher.class.getClassLoader().getResourceAsStream(conf));
            } else {
            ...
            final JettyEmbedServer server = new JettyEmbedServer(properties.getProperty("*.jetty", "jetty.xml"));
            server.start();
